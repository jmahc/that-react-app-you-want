...
### webpack-dashboard
You will notice some kind of `this.input.charAt()` error when running this application with `webpack-dashboard`.  This is an issue since the upgraded Webpack 3.  You can do without `webpack-dashboard` and notice that no errors occur.  Otherwise, ignore them you pretentious asshole.
